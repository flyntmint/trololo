...
const nukeWheelSpinButton = document.getElementById("nuke-wheel-spin");
const nukeWheelResult = document.getElementById("nuke-wheel-result");
const finish = document.getElementById("finish");
const finishTitle = document.querySelector("#finish h1");
const finishMessage = document.querySelector("#finish p");
const finishBackButton = document.getElementById("finish-back");
const healthLabel = document.getElementById("health-label");
const healthFill = document.getElementById("health-fill");
const shopPanel = document.querySelector("aside.shop");
const spentLabel = document.getElementById("spent");
const buyGunButton = document.getElementById("buy-gun");
const buyJetpackButton = document.getElementById("buy-jetpack");
const buySpeedButton = document.getElementById("buy-speed");
const mutePopupsButton = document.getElementById("mute-popups");
const avatarDefaultButton = document.getElementById("avatar-default");
const avatarMugaButton = document.getElementById("avatar-muga");
const avatarConsoleButton = document.getElementById("avatar-console");
const avatarSaveButton = document.getElementById("avatar-save");
const avatarLoadButton = document.getElementById("avatar-load");
const avatarStatus = document.getElementById("avatar-status");
const bgDefaultButton = document.getElementById("bg-default");
const bgMugaButton = document.getElementById("bg-muga");
const bgDarkButton = document.getElementById("bg-dark");
const bgDonutButton = document.getElementById("bg-donut");
const bgStatus = document.getElementById("bg-status");
const scamButton = document.getElementById("scam-btn");
const nukeButton = document.getElementById("nuke-btn");
const donatePopup = document.getElementById("donate-popup");
const donateCloseButton = document.getElementById("donate-close");
const donateOptionButtons = Array.from(document.querySelectorAll(".donate-option"));
const robuxPopup = document.getElementById("robux-popup");
const robuxBuyButton = document.getElementById("robux-buy");
const robuxCloseButton = document.getElementById("robux-close");
const annoyPopup = document.getElementById("annoy-popup");
const annoyCloseButton = document.getElementById("annoy-close");
const loadingOverlay = document.getElementById("loading-overlay");
const loadingFill = document.getElementById("loading-fill");
const loadingText = document.getElementById("loading-text");
const loadingSkipButton = document.getElementById("loading-skip");
const loadingPauseButton = document.getElementById("loading-pause");
...
const authSavePasswordButton = document.getElementById("auth-save-password");
const authForgotButton = document.getElementById("auth-forgot");
const authNewAccountButton = document.getElementById("auth-new-account");
...
const serverCreateButton = document.getElementById("server-create");
const serverJoinButton = document.getElementById("server-join");
const serverList = document.getElementById("server-list");
const serverMessage = document.getElementById("server-message");
const payMethodButtons = Array.from(document.querySelectorAll(".pay-method"));
const muteButton = document.getElementById("mute-btn");
const sheepMuteButton = document.getElementById("sheep-mute-btn");
const skipRageButton = document.getElementById("skip-rage-btn");
const modeOverlay = document.getElementById("mode-overlay");
const modeSuperDuperMugaEasyButton = document.getElementById("mode-super-duper-muga-easy");
const modeSuperEasyButton = document.getElementById("mode-super-easy");
const modeEasyButton = document.getElementById("mode-easy");
const modeMediumButton = document.getElementById("mode-medium");
const modeMugaButton = document.getElementById("mode-muga");
const modeDangerButton = document.getElementById("mode-danger");
const mugaModePopup = document.getElementById("muga-mode-popup");
const mugaModeCloseButton = document.getElementById("muga-mode-close");
const coolAd = document.getElementById("cool-ad");
const adminPanel = document.getElementById("admin-panel");
const adminToggleButton = document.getElementById("admin-toggle");
const adminInfiniteButton = document.getElementById("admin-infinite");
const adminPowerButton = document.getElementById("admin-power-btn");
const adminStatus = document.getElementById("admin-status");
const endingsPanel = document.getElementById("endings-panel");
const endingsButton = document.getElementById("endings-btn");
const endingsList = document.getElementById("endings-list");
const bankruptButton = document.getElementById("bankrupt-btn");
const shopToggleButton = document.getElementById("shop-toggle-btn");
...
let currentRageVariant = "regular";
...
const endingOrder = ["good", "bad", "fish", "money", "impatient", "boom", "bankrupt", "muga"];
const endingLabels = {
  good: "Good Ending",
  bad: "Bad Ending",
  fish: "Fish Ending",
...
const spikes = [];
const trollParts = [];
let activeFishAttack = null;

const headImage = new Image();
headImage.src = "./src/trololo-head.png?v=3";
let processedHeadCanvas = null;
...
async function playBgmFromSources(sources, loop) {
...
function updateSheepMuteButtonLabel() {
...
function serverStorageKeyForUser(username) {
  return `${serverListStoragePrefix}${username.toLowerCase()}`;
}

function readServersForUser(username) {
...
function writeServersForUser(username, servers) {
...
function renderServerList() {
  if (!serverList || !activeUsername) return;
...
function showServerSelection(username) {
...
function updateAuthUI() {
  ...
  if (stored) {
    ...
    if (authHint) authHint.textContent = `Use your account: ${stored.username}`;
    if (authSubmitButton) authSubmitButton.textContent = "Log In";
  } else {
    if (authTitle) authTitle.textContent = "Create Account";
    if (authHint) authHint.textContent = "Create username + password to play.";
    if (authSubmitButton) authSubmitButton.textContent = "Create Account";
  }
}

function startPostAuthFlow() {
  authenticated = true;
  if (serverHint) {
  ...
  if (loadingText) loadingText.textContent = "preparing rage content";
  if (loadingPauseButton) loadingPauseButton.textContent = "Pause Loading";
}
...
function buildTransparentHeadSprite() {
...
function getAvatarKeyFromStorage(value) {
...
function getCurrentAvatarDrawable() {
...
function maybeTriggerMugaEnding() {
...
function circleIntersectsRect(circleX, circleY, circleRadius, rectX, rectY, rectW, rectH) {
...
function queueWorldGeneration() {
  while (worldFarX < cameraX + canvas.width + 700) {
    spawnChunk();
  }
}

function buildRagePart(startX) {
  let x = startX;
...
function buildMovingSpikesRagePart(startX) {
  let x = startX;
...
function buildRandomRagePart(startX) {
...
function teleportToRagePart(nowSec) {
...
function maybeTriggerRagePart(nowSec) {
  if (ragePartTriggered || ragePartSkipped || gameOver || !gameStarted) return;
  if (cameraX < 1600 && starsCollected < 18) return;
...
function updateLoadingScreen(nowSec) {
  if (!loadingVisible) return;
...
function skipRagePart() {
  if (!gameStarted || gameOver || skipRageUsed) return;
...
function starShapePath(x, y, outerRadius, innerRadius) {
  const points = 5;
  let angle = -Math.PI / 2;
...
function drawPlatform(platform) {
  ...
  if (x + platform.width < -80 || x > canvas.width + 80) return;
...
function buySpeedUpgrade() {
  if (speedTier >= speedMultipliers.length) return;
...
function toggleAdminInfiniteDollars() {
  adminInfiniteDollars = !adminInfiniteDollars;
  updateAdminPanelUI();
}

function updateMutePopupsButtonLabel() {
...
function updateShopToggleButtonLabel() {
  if (!shopToggleButton) return;
...
function toggleEndingsPanel() {
  if (!endingsPanel) return;
...
function unlockEnding(endingId) {
  if (unlockedEndings[endingId]) return;
...
function triggerBankruptEnding() {
  if (gameOver) return;
...
function damagePlayer(amount, endingType = "bad") {
  if (gameOver) return;
...
function goBackToLoadingScreen() {
  gameStarted = false;
  gameOver = false;
  allEndingsMessageShown = false;
  nukeWheelSpinning = false;
...
function showDonatePopup() {
  if (popupsMuted || donateVisible || robuxVisible || gameOver) return;
...
function showRobuxPopup() {
  if (popupsMuted || robuxVisible || donateVisible || gameOver) return;

  robuxVisible = true;
  robuxPopup?.classList.add("visible");
}

function hideRobuxPopup() {
  if (!robuxVisible) return;
...
function showAnnoyPopup() {
  if (popupsMuted || annoyVisible || donateVisible || robuxVisible || gameOver) return;

  annoyVisible = true;
  annoyPopup?.classList.add("visible");
}

function hideAnnoyPopup() {
  if (!annoyVisible) return;
...
function showMugaModePopup() {
  mugaModePopup?.classList.add("visible");
}

function hideMugaModePopup() {
  mugaModePopup?.classList.remove("visible");
}

function updateCoolAd(nowSec) {
...
function scheduleNextReward(fromSec) {
  nextRewardAt = fromSec + random(60, 600);
}

function applyStarReward(amount) {
...
function maybeGrantTimedReward(nowSec) {
  if (gameOver || donateVisible || nowSec < nextRewardAt) return;
...
function updateFishAttack(dt) {
  if (!activeFishAttack || gameOver) return;
  ...
  const hit = circleIntersectsRect(fish.x, fish.y, fish.size * 0.3, player.x, player.y, player.width, player.height);
...
function updateSpikeDamage(nowSec) {
  if (gameOver) return;

  if (nowSec - lastSpikeHitAt < 0.55) return;

  for (const spike of spikes) {
    const hit = circleIntersectsRect(player.x + player.width / 2, player.y + player.height - 8, 14, spike.x, spike.y, spike.width, spike.height);
    if (hit) {
...
function updateMovingSpikes(nowSec) {
  for (const spike of spikes) {
    if (
...
function triggerTrollPart(trollPart) {
...
function updateTrollParts() {
  if (gameOver) return;

  for (const trollPart of trollParts) {
    if (trollPart.triggered) continue;
...
function updateStarsAndParticles(dt) {
...
function drawSpikes() {
  for (const spike of spikes) {
    ...
    if (x + spike.width < -40 || x > canvas.width + 40) continue;
...
function drawTrollParts() {
  for (const trollPart of trollParts) {
    const x = trollPart.x - cameraX;
    if (x + trollPart.width < -40 || x > canvas.width + 40) continue;
...
function drawFishAttack() {
  if (!activeFishAttack) return;
...
window.addEventListener("resize", () => {
  ...
  if (prevHeight !== 0) {
    ...
    for (const platform of platforms) platform.y += delta;
    for (const star of stars) star.y += delta;
    player.y += delta;
  }
});
...
for (const button of payMethodButtons) {
  button.addEventListener("click", () => {
    donatePayMethod = button.dataset.method ?? "card";
